...
var recordingBtn = recordingDIV.querySelector("button");
recordingBtn.onclick = function() {
  ...
  if (recordingMedia.value === "record-audio-plus-video") {
    ...
    button.mediaCapturedCallback = function() {
      if (DetectRTC.browser.name != "Firefox" && DetectRTC.browser.name != "Chrome") {
        // opera or chrome etc.
        button.recordRTC = [];
...
function captureScreen(config) {
  getScreenId(function(error, sourceId, screenConstraints) {
    if (error === "not-installed") {
      document.write(
        '<h1><a target="_blank" href="https://chrome.google.com/webstore/detail/screen-capturing/ajhifddimkapgcifgcodmmfdlknahffk">Please install this chrome extension then reload the page.</a></h1>'
      );
    }
...
if (DetectRTC.browser.name === "Edge") {
  ...
  console.warn(
    "Ni la API de MediaRecorder ni webp son compatibles con Microsoft Edge. Simplemente puedes grabar audio."
  );
...
if (DetectRTC.browser.name === "Chrome") {
  ...
  console.info(
    "Esta demostración de RecordRTC simplemente intenta reproducir la sincronización de audio / video dentro del navegador. Todavía genera dos archivos separados (WAV / WebM)."
  );
}
...
function saveToDiskOrOpenNewTab(recordRTC) {
  ...
  uploadToServer(recordRTC, function(progress, response) {
    if (progress === "ended") {
      toogleOverlay(false);
      $("#id").val(response.id);
      Proximo();
...
function uploadToServer(recordRTC, callback) {
  ...
  const ejes = serial.find(el => el.name === "ejes").value;
  formData.append("ejes", ejes);
  ...
  var upload_url = baseURL + "/proposiciones";
...
function makeXMLHttpRequest(url, data, callback) {
  ...
  request.onloadend = function() {
    ...
    callback("upload-ended", response);
  };
